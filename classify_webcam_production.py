"""
PRODUCTION Sign Language Recognition System
- Tracks hand automatically (no fixed box)
- Tolerates natural hand movements
- Stable 3-second hold with motion detection
- Won't reset on small shakes
- Professional UX
"""
...
def calculate_hand_motion(landmarks_history):
    """
    Calculate how much the hand is moving
    Returns motion score (0 = still, 1 = moving a lot)
    """
    if len(landmarks_history) < 2:
        return 0.0
    
    # Compare last two hand positions
    ...
    # Normalize to 0-1 range (0.05 is significant movement)
    motion_score = min(avg_movement / 0.05, 1.0)
    
    return motion_score


def analyze_predictions(prediction_buffer, min_agreement=0.5):
    """Analyze predictions with voting"""
    
    if len(prediction_buffer) < 15:  # Need at least 15 frames (~0.5 seconds)
...
def main():
    ...
    print("="*70)
    print("PRODUCTION Sign Language Recognition System")
    print("="*70)
    print("\nFeatures:")
    print("  ✓ Automatic hand tracking (no fixed box)")
    print("  ✓ Motion-tolerant (won't reset on small movements)")
    print("  ✓ 3-second stable hold requirement")
    print("  ✓ Multi-frame voting for accuracy")
    ...
    print("\nStarting camera...")
    print("\nHow it works:")
    print("  1. Show your hand - system will track it automatically")
    print("  2. Make a sign and hold it STEADY for 3 seconds")
    print("  3. Small movements are OK - system detects sign changes")
    print("  4. Timer fills up when hand is stable")
    print("  5. Letter is added when timer completes")
    print("\nControls:")
    print("  - ESC: Exit")
    ...
    with tf_v1.Session() as sess:
        ...
        # Hold timer
        HOLD_TIME = 3.0
        MOTION_THRESHOLD = 0.3  # Motion above this resets timer
        STABILITY_WINDOW = 0.5  # Must be stable for 0.5s before starting timer
        ...
        while True:
                        ...
                        try:
                            ...
                            if hold_duration >= HOLD_TIME:
                                ...
                                final_pred, final_conf, reason = analyze_predictions(
                                    prediction_buffer, min_agreement=0.45
                                )
            ...
            # Info overlay
            overlay = frame.copy()
            cv2.rectangle(overlay, (0, 0), (700, 320), (0, 0, 0), -1)
            ...
            if is_holding:
                ...
                status_text = "Hold hand steady to start timer"
            ...
            if is_holding and prediction_buffer:
                ...
                cv2.putText(frame, f"Analyzing: {temp_pred.upper()}", (10, 140), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1.0, pred_color, 2)
                cv2.putText(frame, f"Agreement: {temp_agreement:.0f}%", (10, 175), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, pred_color, 2)
                
                # Top 3
                cv2.putText(frame, "Top 3:", (10, 210), 
            ...
            cv2.imshow('Sign Language Recognition - PRODUCTION', frame)
