"""
Sign Language Recognition - Working Version with TF1
Uses MediaPipe for hand detection + TensorFlow 1.x for classification
"""
...
# Import TensorFlow 1.x (not compatibility mode)
...
def extract_hand_region(image, hand_landmarks, padding=60):
    """Extract hand region based on MediaPipe landmarks"""
    h, w = image.shape[:2]
...
def main():
    ...
    print("="*70)
    print("Sign Language Recognition - TensorFlow 1.x Version")
    print("="*70)
    print("\nThis version uses:")
    print("  - MediaPipe for hand detection")
    print("  - TensorFlow 1.x for sign classification")
    print("  - Works with existing trained model!")
    print("\nLoading models...")
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            # FPS
            cv2.putText(frame, "FPS: {:.1f}".format(avg_fps), (10, 65), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            
            # Model info
            cv2.putText(frame, "TensorFlow 1.x + MediaPipe", (10, 95), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            
            # Prediction
            if res:
                cv2.putText(frame, "Sign: {}".format(res.upper()), (10, 130), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 3)
                cv2.putText(frame, "Confidence: {:.1f}%".format(score*100), (10, 165), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            
            cv2.imshow('Sign Language Recognition - TF1', frame)
